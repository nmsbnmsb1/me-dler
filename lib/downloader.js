...
class Downloader extends me_actions_1.RunOne {
    constructor(context) {
        super(me_actions_1.ErrHandler.RejectAllDone);
        this.context = context;
    }
    async doStart(context) {
        if (fs_1.default.existsSync(context.file))
            return;
        let one = new me_actions_1.RunOne(me_actions_1.ErrHandler.RejectImmediately);
        {
            one.addChild(new init_context_1.default());
            one.addChild(new file_open_1.default());
            one.addChild(new meta_reader_1.default());
            one.addChild(new me_actions_1.ActionForFunc(async () => {
                if (context.metaData.status) {
                    one.addChild(new head_request_1.default());
                    one.addChild(new threads_generator_1.default());
                    one.addChild(new meta_writer_1.default());
                }
                one.addChild(new data_request_1.default());
            }));
        }
        this.addChild(one);

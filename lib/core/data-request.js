...
class default_1 extends me_actions_1.Action {
    async doStart(context) {
        ...
        if (metaData.threads.length <= 1) {
            this.request = new threads_request_1.default(metaData.threads[0]);
        }
        else {
            let runAll = new me_actions_1.RunAll(me_actions_1.ErrHandler.RejectAllDone);
            for (let thread of metaData.threads) {
                if (thread.position < thread.end)
                    runAll.addChild(new threads_request_1.default(thread));
            }
            this.request = runAll;
        }

...
class default_1 extends me_actions_1.Action {
    ...
    async doStart(context) {
        ...
        try {
            this.response = await (0, utils_1.request)({ method: context.method, url: runtime.url, headers, timeout: context.timeout, responseType: 'stream' });
            this.onData = (chunk) => {
                if (!this.isPending())
                    return;
                try {
                    fs_1.default.writeSync(runtime.fileDescriptor, chunk, 0, chunk.length, this.thread.position);
